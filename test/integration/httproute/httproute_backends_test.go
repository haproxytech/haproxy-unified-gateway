...
func (s *HTTPRouteTestSuite) Test_HTTPRoute_Backend_1_route_filter_ko_cross_level() {
	fixtureDirPath := utils.GetCRDFixturePath()
	fixtureDir := "backends"
	fixturePath := path.Join(fixtureDirPath, fixtureDir, "1_route_filter_ko_cross_level")
	s.CreateFixtures(fixturePath, nil)
	defer s.CleanupFixtures(fixturePath, nil)
...
func (s *HTTPRouteTestSuite) Test_HTTPRoute_Backend_1_route_filter_ko_partial() {
	fixtureDirPath := utils.GetCRDFixturePath()
	fixtureDir := "backends"
	fixturePath := path.Join(fixtureDirPath, fixtureDir, "1_route_filter_ko_partial")
	s.CreateFixtures(fixturePath, nil)
	defer s.CleanupFixtures(fixturePath, nil)
